...
class AuthService {
  ...
  Future<UserModel> uploadAvatar(String filePath) async {
    ...
    final response = await _dio.post(ApiEndpoints.userAvatar, data: formData);
    return UserModel.fromJson(response.data as Map<String, dynamic>);
  }
  ...
  Future<AuthState?> restoreSession() async {
    ...
    try {
      ...
      final cachedUser = UserModel(id: userId, username: username);
      try {
        final freshUser = await getMe();
        return AuthState.loggedIn(freshUser);
      } catch (_) {
        return AuthState.loggedIn(cachedUser);
      }
    } catch (_) {
      return null;
    }
